...
// socket-layer hooks: connect4 + udp4_sendmsg (query redirect) and
// udp4_recvmsg (loopback answer rewrite), sharing one cfg_map and hit_cnt.
//
...
//   SEC("cgroup/udp4_sendmsg")  -> BPF_CGROUP_UDP4_SENDMSG  (attach 14)
//   SEC("cgroup/udp4_recvmsg")  -> BPF_CGROUP_UDP4_RECVMSG  (attach 19)
//
// All three share the cfg_map and keyed hit_cnt declared in dns_cfg.h;
// hit_cnt keys: 0 = connect4, 1 = sendmsg, 2 = recvmsg.
//
// Each rule is a self-contained redirect (requested <-> actual). match_rule()
// takes the direction explicitly:
//   DNS_QUERY  (connect4/sendmsg): match the *requested* nameserver, rewrite
//                                  the destination to the *actual* upstream.
//   DNS_ANSWER (recvmsg)         : match the *actual* source actually seen,
//                                  rewrite the reported source back to the
//                                  *requested* nameserver.
// One rule serves both directions of the same redirect.
//
// The recvmsg hook only rewrites the *reported* source in the socket layer, so
...
{
    if (redirect_query(ctx))
        count_hit(DNS_HIT_SENDMSG);
    return SK_PASS;
}

// Answer direction (udp4_recvmsg): the kernel has delivered a datagram from
